...
namespace Assets.Scripts.UFOGame
{
    ...
    public class UFOGameRule : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // 敵の移動
            manager.AddRule(new Rule(new OneObjectMatch("UFO"), new MoveEffect(transform => transform.forward.normalized * 0.01f)));
            // 敵の射撃
            manager.AddRule(new Rule(new TimerMatching(new OneObjectMatch("UFO"), 3.0f), new GenerateEffect(Prefabs[1])));
            // 弾の移動
            manager.AddRule(new Rule(new OneObjectMatch("Bullet"), new MoveEffect(transform => transform.up.normalized * 0.02f)));
            // 武器と敵の当たり判定
